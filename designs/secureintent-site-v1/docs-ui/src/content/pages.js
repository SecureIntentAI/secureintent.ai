...
const preview = (text) =>
  `<div class="preview-note">${icon("info")}<p><strong>Local design preview.</strong> ${text}</p></div>`;
export const pages = [
  {
    slug: "start",
  ...
  },
  {
    ...
    compact: true,
    body: `
    <div class="account-grid"><div class="account-story"><div class="account-orbit"><div>${icon("shield")}</div><span class="orbit-label label-one">Your device</span><span class="orbit-label label-two">Your control</span></div><h2>Good ideas travel.<br><span>Secrets shouldn’t.</span></h2><p>Keep your momentum. Bring protection into the tools you already use.</p><a class="text-link" href="/designs/secureintent-site-v1/docs/overview/what-is-secureintent/">Get to know SecureIntent ${icon("arrow")}</a></div><div class="account-card"><span class="status-pill">Account preview</span><h1>Welcome back</h1><p class="form-intro">Explore the account experience.</p><p class="selected-plan" hidden></p><form data-preview-form="account"><label for="account-email">Email address</label><input id="account-email" name="email" type="email" placeholder="you@company.com" autocomplete="off" required><button class="button button-mint" type="submit" disabled>Preview sign-in ${icon("arrow")}</button><p class="form-status" role="status"></p></form><div class="form-divider"><span>No account? No problem.</span></div><a class="button button-outline" href="/designs/secureintent-site-v1/docs/demo.html#playground">Try the product example ${icon("arrow")}</a><p class="form-fineprint">This form stays in your browser. Authentication is not connected. No email is sent or stored.</p></div></div>`,
  },
  {
    slug: "contact",
